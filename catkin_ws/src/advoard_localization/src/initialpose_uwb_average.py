...
def subscribe_data_pose(PointStamped):
    ...
    if  poses_final == [] and len(poses_initial)<30:
            poses_initial.append([PointStamped.position.x,PointStamped.position.y])        
    elif control_finish_move == True and len(poses_final)<30 :
            poses_final.append([PointStamped.position.x,PointStamped.position.y])
...
if __name__ == "__main__":
    subscriber =rospy.Subscriber('localization_data_chan_ho_topic', Pose, subscribe_data_pose)
